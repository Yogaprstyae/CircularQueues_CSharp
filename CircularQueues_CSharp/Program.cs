    ...
    class Queues
    {
        ...
        public void insert(int element)
        {
            ...
            /*The following statement checks whether the queue is empty. If the
                queue is empty, then the value of the REAR and FRONT variables is set
                to 0. */
            if (FRONT == -1)
            {
                FRONT = 0;
            ...
            else
            {
                /* If REAR is at the last position of the array, then the value of REAR
                    is set to 0 that corresponds to the firts psotion of the array. */
                if (REAR == max - 1)
                    REAR = 0;
                else
            ...
            /* Once the postion of REAR is determined, the element is added at
                its proper place. */
        ...
        public void display()
        {
            ...
            else
            {
                ...
                FRONT_position = 0;
                /* traverses the queue till the last position of the queue. */
                while (FRONT_position <= REAR_position)
                {
                    Console.Write(queue_array[FRONT_position] + "  ");
                    FRONT_position++;
